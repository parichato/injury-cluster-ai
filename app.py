...
if model_choice == "Predict (CatBoost Multi-Class)":
    ...
    st.caption("โมเดลนี้ใช้ CatBoost ในการทำนายระดับความรุนแรงของผู้บาดเจ็บตามปัจจัยเสี่ยงที่ป้อนด้านซ้าย")

    if st.button("🔍 ทำนายระดับความรุนแรง"):
        # ⚙️ เตรียมข้อมูลก่อนเข้าโมเดล
        X_scaled = X_input  # ถ้าไม่ได้ใช้ scaler_pred ให้ใช้ข้อมูลตรงๆ
        try:
            X_scaled = scaler_pred.transform(X_input)
        except:
            pass  # ป้องกัน error ถ้าไม่มี scaler_pred

        # 🔮 ทำนายด้วยโมเดล CatBoost
        ...
        msg, style = severity_map.get(pred_class, ("ไม่ทราบผลลัพธ์", "info"))
        ...
        if style == "success":
            ...
            st.info("ยังไม่สามารถระบุผลการทำนายได้")
